...
class PickingCrossword:
    ...
    def __init__(self, master, switch_frames):
        ...
        self.label1 = tk.Label(master, text="Word List (comma-separated):")
        self.label1.grid(row=0, column=0)

        self.word_list_entry = tk.Entry(master)
        self.word_list_entry.grid(row=1, column=0)

        self.label2 = tk.Label(master, text="Number of Crosswords:")
        self.label2.grid(row=0, column=1)

        self.crossword_count_entry = tk.Entry(master)
        self.crossword_count_entry.grid(row=1, column=1)
    ...
    def generate_crosswords(self):
        try:
            ...
            for k, crossword in enumerate(generator.get_crosswords()):
                canvas = CrosswordCanvas(self.master, width=generator.get_grid_size()*cell_size, height=generator.get_grid_size()*cell_size+100, crossword=crossword, switch_frames=self.switch_frames)
                ...
                canvas.canvas.create_text(generator.get_grid_size() * cell_size // 2, 10, text=f"Score: {crossword.get_score()}", font=("Arial", 12, "bold"))
                canvas.canvas.create_text(generator.get_grid_size() * cell_size // 2, 30, text=f"Words Used: {', '.join([w.get_word() for w in crossword.get_words_used()])}", font=("Arial", 10))
                canvas.canvas.create_text(generator.get_grid_size() * cell_size // 2, 50, text=f"Words Unused: {', '.join(crossword.get_words_unused())}", font=("Arial", 10))
